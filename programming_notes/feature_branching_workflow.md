...
così che ogni futuro comando `git pull`, quando localmente è attivo il ramo `nuova_funzionalita`, faccia automaticamente riferimento a `origin/nuova_funzionalita`. In altre parole stiamo dicento a `git` di voler mantenere in sincronia la ramificazione locale e quella remota.
